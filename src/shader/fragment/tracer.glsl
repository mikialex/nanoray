...
Triangle getTriangle(int index) {
    float u = float(index) * u3;
    return Triangle(texture2D(uSampler, vec2(u, 0.0)).rgb,
              texture2D(uSampler, vec2(u + u1, 0.0)).rgb,
              texture2D(uSampler, vec2(u + u2, 0.0)).rgb);
}

triangle_getIntersection(int index, vec3 origin, vec3 delta) {
    Triangle triangle = getTriangle(index);

   
}

triangle_getPointNormal(int index, vec3 hit) {
    Triangle triangle = getTriangle(index);
...
bool intersect(
    ...
    // for (int i = 0; i < 64; ++i) {
    //     t = triangle_getIntersection(1 , i, origin, delta);
    //     if (t < hitResult) {
    //         hitResult = t;
    //         hitIndex = i;
    //     }
